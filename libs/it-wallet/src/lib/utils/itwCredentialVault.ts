 ...
 * (SD-JWT / MDOC). Each credential is persisted under its own key, so the
 * payload size of a single read/write stays bounded as the wallet grows.
 *
 * Keys follow the convention `itw:credential:{credentialType}`. Since
 * expo-secure-store rejects `:` in raw keys, the same `:` -> `_` translation
 * used by the secure storage persist engine is applied here.
 *
