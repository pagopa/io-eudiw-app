...
import {Body, H6, IOStyles, VSpacer, VStack} from '@pagopa/io-app-design-system';
...
const PresentationProximityQRCode = ({
  ...
  const proximityDisclosureDescriptor = useAppSelector(selectProximityDisclosureDescriptor);
  ...
  useEffect(() => {
    if (proximityStatus === 'authorization-started' && proximityDisclosureDescriptor) {
      navigation.navigate('MAIN_WALLET_NAV', {screen: 'PROXIMITY_PREVIEW', params : {descriptor : proximityDisclosureDescriptor}});
    }
  ...
  return (
    <View>
      ...
      <VSpacer size={40} />
      { proximityStatus === 'connected' && <VStack space={16} style={IOStyles.alignCenter}>
        <LoadingIndicator size={24} />
        <H6
