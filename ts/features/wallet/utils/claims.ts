 ...
 */
const baseClaimSchemaExtracted = baseClaimSchema.transform((baseClaim) => baseClaim.value)
 ...
 */
const JPEG_SOF_CODES = [0xC0, 0xC1, 0xC2, 0xC3, 0xC5, 0xC6, 0XC7, 0XC8, 0XC9, 0XCA, 0XCB, 0XCD, 0XCE, 0XCF]

/**
 * Schema to validate claims that are known to have a base64url representation of a jpeg in their value,
 * discriminated by the claim labels TODO
 */
export const base64ImageSchema = z
  ...
  .transform(b64url => {
    ...
    const b64 = b64_unpadded.length % 4 == 0 ? b64_unpadded : b64_unpadded + Array.from(Array(4 - b64_unpadded.length % 4).keys()).reduce((prev) => prev + "=", "")
    const { width, height } = Buffer.from(b64, 'base64').reduce((prev, byte, index, buffer) => {
      ...
      if (JPEG_SOF_CODES.includes(byte)) {
        ...
        const height = (buffer as Buffer).readUint16BE(index + 4)
        const width = (buffer as Buffer).readUint16BE(index +6)
        return {
          ...prev,
          height,
          width,
...
export const claimScheme = z.union([
  ...
  baseClaimSchemaExtracted.pipe(z.union([
    dateSchema,
