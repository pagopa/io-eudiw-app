...
type Props = {
  ...
  // Sets if the accordion is open by default
  defaultOpen?: boolean;
};
 ...
 * @constructor
 */
export const ClaimsSelector: React.FC<Props> = props => {
  const [isOpen, setOpen] = useState<boolean>(props.defaultOpen ?? false);
  const animatedController = useRef(
    new Animated.Value(!props.defaultOpen ? 1 : 0)
  ).current;
